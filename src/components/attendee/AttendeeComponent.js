...
class AttendeeComponent extends React.Component {
    ...
    componentDidMount() {
        ...
        if (id === "profile") {
            this.setState({attendee: this.props.attendee, ownProfile: true})
        } else {
    ...
    componentDidUpdate(prevProps, prevState, snapshot) {
        if ((prevState.attendee._id !== this.props.attendee._id || prevState.attendee._id !== this.state.attendee._id) && this._isMounted) {
            const pathParts = window.location.pathname.split('/');
            const id = pathParts.pop() || pathParts.pop();
            if (id === "profile") {
                this.setState({attendee: this.props.attendee, ownProfile: true})
                    ...
                    <div className="row mb-3">
                        {
                            ...
                            <div className="row col-12 d-inline text-center mt-3 mb-3 ">
                                <div>
                                    You haven't liked any events<br/>
